---
description: Locked review gate — cross-model rubric review (Claude self + GPT via openai/gpt-5.6-sol in opencode, then github-copilot/gpt-5.6-sol where a seat is served, then codex fallback) plus dedicated security and leanness passes, over a task's diff or a whole feature. The ONLY reviewer /dev-loop uses.
...
**The bridge chain (every gate, every machine): opencode(openai) → opencode(copilot, where served)
→ codex.** The pinned model is `openai/gpt-5.6-sol` through opencode's OpenAI auth (§ Bridge chain
in `~/.claude/reference/model-policy.md`). Where the machine's opencode serves
`github-copilot/gpt-5.6-sol` (a Copilot seat), that leg sits between openai and codex as the
OpenAI-quota-exhaustion fallback — a **separate seat quota**, so deplete the openai leg first;
where the leg isn't listed it's a no-op. Codex is the final harness fallback if opencode dies.
...
the copilot leg exists only if `opencode models | grep -qx 'github-copilot/gpt-5.6-sol'` — absent →
skip it silently (no-op on machines without a seat), never substitute a lesser copilot model.
...
```bash
timeout 300 opencode run --dir "$REPO" -m openai/gpt-5.6-sol --agent plan --format json \
...
- **Gate model at DEFAULT variant — NEVER add `--variant high`.** Reasoning
...
**opencode copilot leg (OpenAI-quota-exhaustion fallback, where served):** the openai leg dead AND
`github-copilot/gpt-5.6-sol` in the `bridge-ok` marker (or listed by `opencode models`) → the
**identical command with only the model flag changed**: `-m github-copilot/gpt-5.6-sol`. Same
prompt, same timeout, same jq, same laws (stdin, liveness kill, verbatim retry once). This rides
the machine's Copilot seat on metered premium requests — a **separate quota pool** from the openai
leg, which is the whole point of this leg: it's the only fallback that survives an OpenAI-quota
exhaustion (codex below shares the openai quota). Leg not listed on this machine → no-op, skip
straight to codex; never substitute a different copilot model.

**codex leg (final fallback)** — the opencode legs are exhausted (this is also the escape hatch for
